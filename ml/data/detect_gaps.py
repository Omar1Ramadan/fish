...
class AISGapDetector:
    ...
    def detect_gaps_from_json(
        ...
        # GFW API structure: entries[0][dataset_name] = list of vessel records
        dataset_key = list(entries[0].keys())[0] if entries else None
        if not dataset_key:
            print("⚠️ Could not find dataset key in response")
            return []
        
        vessel_records = entries[0][dataset_key]
        print(f"📊 Found {len(vessel_records)} vessel records in dataset '{dataset_key}'")
        
        # Group records by vessel ID (GFW returns flat list with multiple records per vessel)
        vessels_grouped: Dict[str, List[Dict]] = {}
        for record in vessel_records:
            vessel_id = record.get('vesselId') or record.get('mmsi') or record.get('id')
            if not vessel_id:
                continue
            
            if vessel_id not in vessels_grouped:
                vessels_grouped[vessel_id] = []
            
            # Extract position data from each record
            position = {
                'lat': record.get('lat'),
                'lon': record.get('lon'),
                'timestamp': record.get('date') or record.get('entryTimestamp') or record.get('timestamp'),
                'speed': record.get('speed'),
                'course': record.get('course'),
                'flag': record.get('flag'),
                'shipName': record.get('shipName'),
                'vesselType': record.get('vesselType'),
                'geartype': record.get('geartype'),
            }
            vessels_grouped[vessel_id].append(position)
        
        print(f"🚢 Grouped into {len(vessels_grouped)} unique vessels")
        
        # Detect gaps for each vessel
        vessels_with_gaps = 0
        for vessel_id, positions in vessels_grouped.items():
            if len(positions) < 2:
                continue
            ...
            if gaps:
                vessels_with_gaps += 1
                # Add vessel metadata to gaps
                for gap in gaps:
                    gap['flag'] = positions[0].get('flag')
                    gap['shipName'] = positions[0].get('shipName')
                    gap['vesselType'] = positions[0].get('vesselType')
                    gap['geartype'] = positions[0].get('geartype')
                all_gaps.extend(gaps)
        
        print(f"✅ Detected {len(all_gaps)} gap events from {vessels_with_gaps} vessels")
